...
pattern = np.fromfunction(lambda x, y, __ : (x+y)%2, (3, 3, 1))
...
class ClassifierDataset_b(Dataset):
    def __init__(self, examples, labels, transform=None):
        self.examples = []
        self.transform = transform

        for (a, b) in zip(examples, labels):
            if b != 0:
                continue

            self.examples.append(add_pattern(a))
...
class ClassifierDataset_c(Dataset):
    def __init__(self, examples, labels, transform=None):
        self.examples = []
        self.labels = []
        self.transform = transform
...
def perform_train_step(
    ...
    for i, data in enumerate(train_dataloader):
        ...
        if i % real_batch_rate == 0 and False:
...
def create_classifier_dataloaders(
    ...
    val_dataloader_0 = DataLoader(
        ClassifierDataset_c(val_X, val_y, val_transform),
        batch_size=batch_size,
        shuffle=False,
        num_workers=1,
    )
    val_dataloader_1 = DataLoader(
        ClassifierDataset_b(val_X, val_y, val_transform),
        batch_size=batch_size,
        shuffle=False,
        num_workers=1,
    )

    return train_dataloader, [val_dataloader_0, val_dataloader_1]
