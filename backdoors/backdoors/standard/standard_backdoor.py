    ...
    for dataset_name, classifier_name, batch_size, lr in zip(config["datasets"],
        ...
        for backdoor in config["backdoors"]:
            ...
            val_loaders = [DataLoader(v, batch_size=batch_size, num_workers=num_workers,
                        persistent_workers=persistent, shuffle=True) for v in b_vals]
            ...
            for seed in config["seeds"]:
                ...
                classes = 100 if dataset_name == "cifar100" else 10
                classifier = ClassifierWrapper(models[classifier_name](classes), lr=lr,
                ...
                with open(f"{seed}_{dataset_name}_{backdoor['name']}.txt", "w") as f:  # TODO: temp!
                    f.write(str(accs))