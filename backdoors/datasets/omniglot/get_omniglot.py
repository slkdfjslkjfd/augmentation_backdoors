...
def get_omniglot(transform=[transforms.ToTensor()]*2):
    ...
    if not exists("backdoors/datasets/omniglot/omniglot_data.npy"):
    ...
    for c, imgs in enumerate(data[:100]):
        imgs_s = np.random.shuffle(imgs)
        train.extend(list(zip(imgs[15:], np.array([c]*15))))
        test.extend(list(zip(imgs[:15], np.array([c]*5))))
