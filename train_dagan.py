...
#g = dagan_generator = torch.load("g.pt", map_location=device)
#d = dagan_generator = torch.load("d.pt", map_location=device)
...
train_dataloader_0 = create_dagan_dataloader(
    raw_data, num_training_classes, train_transform, batch_size, 0
)
train_dataloader_1 = create_dagan_dataloader(
    raw_data[:3], num_training_classes, train_transform, batch_size, args.bdprop
)

g_opt = optim.Adam(g.parameters(), lr=0.0001, betas=(0.0, 0.9))
d_opt = optim.Adam(d.parameters(), lr=0.0001, betas=(0.0, 0.9))

val_data = raw_data[:num_val_classes]
...
trainer.hydrate_checkpoint("g.pt")
#trainer.train(data_loader=train_dataloader_0, epochs=epochs, val_images=flat_val_data)
trainer.train(data_loader=train_dataloader_1, epochs=epochs+args.iters, val_images=flat_val_data)

# Save final generator model
torch.save(trainer.g, final_generator_path)
#torch.save(trainer.d, "d.pt")
